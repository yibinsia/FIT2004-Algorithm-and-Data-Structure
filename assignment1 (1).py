...
def function1(fitmons, dp):
    ...
    """
    if (dp.get(str(fitmons))):
        return dp.get(str(fitmons))
    
    # base case: if there is only one fitmon, then return its cuteness score
    if (len(fitmons) == 1):
        return [fitmons[0]]
    
    answer = []
    for i in range(len(fitmons)):
        # if i is the first fitmon in the list, then combine it with the optimal fitmon from the right.
        if (i == 0):
            rightMax = function1(fitmons[i+1 ::], dp)
            dp[str(fitmons[i+1 ::])] = rightMax
            answer.append(
            [fitmons[i][0], 
            int(fitmons[i][1] * fitmons[i][2] + rightMax[0][0] * rightMax[0][1]), 
            rightMax[0][2]]
            ) # combine two fitmons into one 

        # if i is the last fitmon in the list, then combine it with the optimal fitmon from the left.
        elif (i == len(fitmons) - 1):
            leftMax = function1(fitmons[0:i], dp)
            dp[str(fitmons[0:i])] = leftMax
            answer.append(
            [leftMax[0][0], 
            int(fitmons[i][1] * fitmons[i][0] + leftMax[0][2] * leftMax[0][1]), 
            fitmons[i][2]]
            ) # combine two fitmons into one  
        # if i is in the middle of the list, then combine it with the optimal fitmons from the left and right.
        else:
            temp = []
            leftMax = function1(fitmons[0:i], dp)
            dp[str(fitmons[0:i])] = leftMax
            ...
            # choose the smaller affinity to combine first, left with the larger affinity, to avoid after multiply 
            # the affinity at last the cuteness score becomes much smaller
            if (fitmons[i][2] < fitmons[i][0]): # if right affinity < left affinity
                temp.append(
                [fitmons[i][0], 
                int(fitmons[i][1] * fitmons[i][2] + rightMax[0][0] * rightMax[0][1]), 
                rightMax[0][2]]
                ) # combine i and right fitmon
                
                answer.append(
                    [leftMax[0][0],
                    int(temp[0][0] * temp[0][1] + leftMax[0][2] * leftMax[0][1]),
                    temp[0][2]
                    ]
                ) # combine i and left fitmon
            
            else: 
                temp.append(
                [leftMax[0][0], 
                int(leftMax[0][2] * leftMax[0][1] + fitmons[i][0] * fitmons[i][1]), 
                fitmons[0][2]]
                ) # combine i and left fitmon
                
                answer.append(
                    [temp[0][0],
                    int(temp[0][2] * temp[0][1] + rightMax[0][0] * rightMax[0][1]),
                    rightMax[0][2]
                    ] # combine i and right fitmon
                )
    ...
    max_answer_index = 0  
    cuteness_score = 0
    # loop the list to find the best cuteness score
    for i in range(len(answer)):
        if (answer[i][1] > cuteness_score):
            max_answer_index = i
            cuteness_score = answer[i][1]
    # set the current optimal combination of fitmons to the dp dictionary
    dp[str(fitmons)] = [answer[max_answer_index]]
    return dp.get(str(fitmons))
